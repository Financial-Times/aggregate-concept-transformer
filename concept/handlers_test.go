...
func TestHandlers(t *testing.T) {
	testCases := map[string]struct {
		...
		resultBody    string
		err           error
		concepts      map[string]transform.OldConcordedConcept
...
func NewMockService(concepts map[string]transform.OldConcordedConcept, notifications []sqs.ConceptUpdate, healthchecks []fthealth.Check, err error) Service {
	return &MockService{
		concepts:      concepts,
...
func (s *MockService) GetConcordedConcept(ctx context.Context, UUID string, bookmark string) (transform.OldConcordedConcept, string, error) {
	if s.err != nil {
		return transform.OldConcordedConcept{}, "", s.err
	}
